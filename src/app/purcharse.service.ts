...
export class PurcharseService {
  ...
  getEmployeeListCanceladas(id:number): Observable<any> {
    const headers=new HttpHeaders({Authorization:'Basic ' + btoa(this.username+ ':' +this.password)})
    return this.http.get(`${this.baseUrl}ic/api/integration/v1/flows/rest/ERP_PO_GET_ALL_PURC_ORDE_CANCELA/1.0/purcharse_order/${id}`,{headers});
  }

  getEmployeeLisRecibidas(id:number): Observable<any> {
    const headers=new HttpHeaders({Authorization:'Basic ' + btoa(this.username+ ':' +this.password)})
    return this.http.get(`${this.baseUrl}${id}ic/api/integration/v1/flows/rest/ERP_PO_GET_ALL_PURC_ORDE_RECI/1.0/purcharse_order/${id}`,{headers});
  }


  getEmployeeListFac(id:number): Observable<any> {
    const headers=new HttpHeaders({Authorization:'Basic ' + btoa(this.username+ ':' +this.password)})
    return this.http.get(`${this.baseUrl}ic/api/integration/v1/flows/rest/ERP_PO_GET_ALL_PURC_ORDE_FACT/1.0/purcharse_order/${id}`,{headers});
  }
  ...
  updateStatusPurchar(id: number): Observable<Object> {
    const headers=new HttpHeaders({Authorization:'Basic ' + btoa(this.username+ ':' +this.password)})
    return this.http.delete(`${this.baseUrl}ic/api/integration/v1/flows/rest/ERP_PO_UPDATE_STATUS_PURCHA/1.0/status_purcharse/${id}`,{headers});
