...
export class ConciliacionService {
  ...
  getEmployee(id: number): Observable<any> {
    ...
    return this.http.get(`${this.baseUrl}ic/api/integration/v1/flows/rest/ERP_AP_PAYMEN_RECORD_DETALL/1.0/payment_record/${id}`,{headers});
  }

  updateEmployee(id: number, value: any): Observable<Object> {
    const headers=new HttpHeaders({Authorization:'Basic ' + btoa(this.username+ ':' +this.password)})
    return this.http.put(`${this.baseUrl}ic/api/integration/v1/flows/rest/ERP_AP_PAYMEN_RECORD_UPDATE/1.0/payment_record/${id}`, value,{headers});
  }


  deleteEmployee(id: number): Observable<any> {
    const headers=new HttpHeaders({Authorization:'Basic ' + btoa(this.username+ ':' +this.password)})
    return this.http.delete(`${this.baseUrl}ic/api/integration/v1/flows/rest/ERP_AP_PAYMEN_RECORD_DELETE/1.0/payment_record/${id}`, { responseType: 'text', headers});
  }
